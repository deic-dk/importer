...
/**
 * This class manages downloader HTTP downloads from the Internet Archive. 
 */
...
class OC_downloaderIA extends OC_downloaderHTTP {
  ...
  private static function setCookieFiles(){
  	$fs = self::getStorage();
  	self::$TEST_COOKIE_FILE = $fs->getLocalFile(OC_downloader::getDownloadFolder()."/.ia-test-cookie.txt");
  	self::$COOKIE_FILE = $fs->getLocalFile(OC_downloader::getDownloadFolder()."/.ia-cookies.txt");
  }
  ...
  private static function cookie_login($user_info){
	...
	public static function lsDir($folderurl, $user_info){
		...
		exec($cmd, $out, $ret);
		if(!empty($out)){
			OC_Log::write('downloader', "Redirect: ".$out[0], OC_Log::WARN);
			$folderurl = $out[0];
		}
	...
	protected static function getAuthHeader($user_info){
		...
		exec($cmd, $out, $ret);
		OC_Log::write('downloader',"Cookies: ".join($out), OC_Log::WARN);
