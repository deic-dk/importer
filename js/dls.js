...
function checkFiles() {
  if( $('#fileList .elts').length == $('#fileList .elts .found').length) {
	$('#geturl').removeClass('btn-default').addClass('btn-primary').css('opacity','1');
  } else {
	$('#geturl').removeClass('btn-primary').addClass('btn-default').css('opacity','0.3');
  }
}
...
function remove_eltdelete(my_elt){
  var len = $('#dllist tr.elts').length;
  var b = my_elt.prev();
  if(my_elt.find('.addelt').is(":visible")){
	//    b.find('.dling').before('<span class="addelt" title="Add another download"><i class="icon icon-plus"></i></span>');
	b.find('.addelt').removeClass('hidden');
	/*	  b.find('.addelt').bind('click',function(){
		  addDownload(true);
		  var first_elt = get_first_elt();
		  if($(this).parent().attr('id')==first_elt.attr('id')){
		  first_elt.find('.dling').first().before('<span class="eltdelete" title="Remove this download"><i class="icon icon-minus"></i></span>');
		  first_elt.find('span.eltdelete').first().bind('click',function(el){
  ...
  if(len==2){
	var first_elt = get_first_elt();
	first_elt.find('.eltdelete').first().addClass('hidden');
  }
  checkFiles();
}
...
function addDownload(d, newurl, newprov, newpreserve){
  ...
  ++elt_num;
  //	$('#dllist #destination').before('<div id="elt_'+parseInt(elt_num+1)+'" class="elts new">'+$('#hiddentpl').html()+'</div>');
  $('#dllist table tbody').append('<tr id="elt_'+parseInt(elt_num+1)+'" class="elts new">'+$('#hiddentpl tbody tr').html()+'</tr>');
  ...
  $("#elt_"+parseInt(elt_num+1)+" .addelt").bind('click',function(){
	...
	var first_elt = get_first_elt();
	var first_id = get_first_id();
	first_elt.find('span.eltdelete').removeClass('hidden');
	/*		if($(this).parent().attr('id')==first_id){
	//		  first_elt.find('.dling').first().before('<span class="eltdelete" title="Remove this download"><i class="icon icon-minus"></i></span>');
	first_elt.find('span.eltdelete').removeClass('hidden');
	first_elt.find('span.eltdelete').first().bind('click',function(){
  ...
  if(d){
	$('#elt_'+aa+' span.eltdelete').bind('click',function(){
	  remove_eltdelete($('#elt_'+aa));
	});
  }
  else{
	$('#elt_'+aa+' span.eltdelete').addClass('hidden');
  }
}
...
function getProvider(msg){
  ...
  /* Code commented out by Christian 19/3/2015
	 ...
	 */
  //`Following code replaces th above
  var p = msg.find('a.pr-value').attr('data-id');
  var u = msg.find('input.url').val();
	  ...
	  $.ajax({
		...
		success:function(s){
		  if(s.e){
			...
			var iframeUri = OC.linkTo('importer','providers/'+s.n+'.php?u='+u+'&p='+p+'&k='+(msg.find('input.slider-check').attr("checked")?1:0)+'&o=1&d='+$("div#destination input.url").val() );
			iframe.load(function(){
			  ...
			  if(iframe[0].contentWindow.getPbPercentDone()==100){
				...
				if(msg.attr('id')==first_id){
				  ...
				  msg.find('span.urlc input.url').removeClass('found');
...
function updateHistory(clear){
  ...
  $.ajax({
	...
	async:true,
	success:function(s){
	  $('#histtable tbody').html('');
	  if(!clear){
		$.each(s.h, function(k,v){
		  $('#histtable tbody').append('<tr><td class="col1">'+v.dl_file+'</td><td class="col2">'+v.dl_ts+'</td><td class="col3">'+v.dl_status+'</td></tr>');
		});
	  } else {
		$('#histtable tbody').append('<tr><td colspan="3"><span class="nohist">No history for now</td></tr>');
	  }
	},
	error:function(){
...
function addFirstDownload(v, myprov, mypreserve){
  ...
  if(myinp.val()==undefined || myinp.val()==""){
	...
	mysel.find('span.eltdelete').remove();
	return true;
  }
  return false;
}
	...
	success:function(s){
	  ...
	  $('.elts .urlc input.url').each(function(el){
		if($(this). val().trim()!=""){
		  ...
		  //			$("#geturl").removeAttr("disabled");
		  //		  }
		  if($('#geturl').hasClass('btn-default')){
			$('#geturl').removeClass('btn-default').addClass('btn-primary').css('opacity','1');
		  }
		  if($("#savelist").attr("disabled")=="disabled"){
			$("#savelist").removeAttr("disabled");
...
/* 
 * This is frederiks original loadFolderUrl()
 *
...
function checkProviderAuth(url,provider){
  $.ajax({
	type:'POST',
	url:OC.linkTo('importer','ajax/getProvider.php'),
	dataType:'json',
...
function loadFolderUrl(myurl,myprov){
  if(myprov==0){
	$('#elt_0 select.chzen-select option').each(function(el){
	  if($(this).text().toLowerCase()==myurl.replace(/^(\w+):\/\/.*$/, "$1").toLowerCase() ||
		  $(this).text().toLowerCase()==myurl.replace(/https:\/\//, "http://").replace(/^(\w+):\/\/.*$/, "$1").toLowerCase()){
	  ...
	  folder_prov = myprov;
	  if(!importer_pw_ok){
		checkProviderAuth(myurl,myprov);
	  }
	}
	});
	if(importer_pw_ok || myprov===0){
...
$(document).ready(function(){
  ...
  $("#clearList").button({text:true}).bind('click',function(){
	...
	if(first_elt.find('.addelt').length==0){
	  var add_elt = first_elt.find('.dling').first().before('<span class="addelt" title="Add another download"><i class="icon icon-plus"></i></span>');
	...
	elt_num = 0;
	$('#fileList').find('span.urlc input.url').removeClass('found').removeClass('notfound');
  ...
  $(".addelt").bind('click',function(){
	...
	var first_elt = get_first_elt();
	var first_id = get_first_id();
	if($(this).parents('tr').attr('id')==first_id){
...
$('#fileList').keypress('.urlc input.url', function(e){
  if(e.which == 13){		        
  	$('#destination input.url').focus();
  }
});


$('#fileList').on('focusout', '.urlc input.url', function(){
  $(this).removeClass('found').removeClass('notfound').addClass('loading');		
  var url = $(this).val();
  $.ajax({
	context: this,
	url:OC.linkTo('importer','ajax/checkURL.php'),
	async: false,
	dataType: 'json',
	data: {url:url},
	type: 'GET',
	success: function(e)
	{
	  if(e.message === null) {
		...
		if(e.message.indexOf('404') == -1) {
		  // Try to guess if the url is a directory
		  var dir = false;
		  if(url.substr(-1) === '/') {
			dir = true;
		  }
		  if(e.message.indexOf('301') > -1){
		    dir = true; 
		  } 
		  if(dir===true) {
			loadFolderUrl(url, $(this).parents('tr').find('a.pr-value').attr('data-id'));
		  	$(this).parents('tr').remove();
		  	$('#fileList tr').each(function(){
			  $(this).find('input.url').removeClass('loading').addClass('found');
			})
			$('#fileList tr').last().find('.addelt').toggleClass('hidden');
		  }


		  $(this).removeClass('loading').addClass('found');
		  $(this).parents('tr').find('.addelt').toggleClass('hidden');
		} else {
		  $(this).removeClass('loading').addClass('notfound');
		}
	  }
	  checkFiles();
	}
